...
class Settings:
    # ----------------- Базовий режим -----------------
    env: str = os.getenv("ENV", "prod").lower()          # "prod" / "dev"
    is_dev: bool = os.getenv("ENV", "prod").lower() == "dev"

    # Префікс у відповідях
    response_prefix: str = os.getenv(
        "RESPONSE_PREFIX",
        "[DEV]" if os.getenv("ENV", "prod").lower() == "dev" else ""
    )
    ...
    PHONE_REMINDER_MIN_INTERVAL_MIN: int = int(
        os.getenv("PHONE_REMINDER_MIN_INTERVAL_MIN", "720")
    )
    PHONE_REMINDER_POLL_EVERY_MIN: int = int(
        os.getenv("PHONE_REMINDER_POLL_EVERY_MIN", "10")
    )
    PHONE_REMINDER_BATCH: int = int(
        os.getenv("PHONE_REMINDER_BATCH", "50")
    )
    ...
    ENOTE_ODATA_URL: str = (
        os.getenv("PROD_ENOTE_ODATA_URL", "").rstrip("/")
        if os.getenv("ENV", "prod").lower() == "prod"
        else os.getenv("DEV_ENOTE_ODATA_URL", "").rstrip("/")
    )
    ...
    ENOTE_API_BASE: str = (
        os.getenv("PROD_ENOTE_API_BASE", "").rstrip("/")
        if os.getenv("ENV", "prod").lower() == "prod"
        else os.getenv("DEV_ENOTE_API_BASE", "").rstrip("/")
    )

    ENOTE_API_URL: str = (
        os.getenv("PROD_ENOTE_API_URL", "").rstrip("/")
        if os.getenv("ENV", "prod").lower() == "prod"
        else os.getenv("DEV_ENOTE_API_URL", "").rstrip("/")
    )

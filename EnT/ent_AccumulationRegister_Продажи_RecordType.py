...
def get_last_period():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT MAX(Period) FROM ent_AccumulationRegister_Продажи_RecordType")
    last_period = cursor.fetchone()[0]
    cursor.close()
    conn.close()
    
    if last_period:
        return last_period - timedelta(days=0)
    return datetime(2010, 1, 1)  # Якщо немає записів, 20 липня 2024 року
...
def fetch_and_store_data():
    ...
    while True:
        batch_size = random.randint(1000, 2000)  # Генеруємо випадкову кількість записів
